...
# 	@load "/slimdata/rafaeldata/brain_cond_data/new_brain_full_water_no_rho_isic_grad_iter_2_ind_"*string(i)*".jld2" Y_train m0  

# 	grad_train[:,:,:, i] = mean(Y_train;dims=3)
# 	m0_train[:,:,:, i] = m0'
# end

# @save "/slimdata/rafaeldata/brain_cond_data/new_grad_water_no_rho_isic_iter_2.jld2" grad_train m0_train 
...
# Plotting configs
sim_name = "cond-net-norho-iter-2-new"
...
m0_train = JLD2.jldopen("/slimdata/rafaeldata/brain_cond_data/grad_water_no_rho_no_isic_iter_2.jld2", "r")["m0_train"];
m0_train = permutedims(m0_train,(2,1,3,4))
grad_train = JLD2.jldopen("/slimdata/rafaeldata/brain_cond_data/grad_water_no_rho_no_isic_iter_2.jld2", "r")["grad_train"];
...
for e=1:n_epochs # epoch loop
    ...
    if(mod(e,plot_every)==0) 
	    ...
	    for (test_x, test_y, test_x0, file_str) in [[X_train,Y_train, X0_train, "train"], [X_test, Y_test, X0_test, "test"]]
			...
			tight_layout()
		  fig_name = @strdict decay_every decay_start decay_fac isic use_dm use_m  flip_aug vmax_d  noise_lev_x n_train e 
    ...
    if(mod(e,save_every)==0) 
		...
		global save_dict = @strdict decay_every decay_start decay_fac isic use_dm iter unet_model  clipnorm_val n_train e noise_lev_x lr n_hidden L K Params loss logdet_train l2_cm ssim loss_test logdet_test l2_cm_test ssim_test batch_size; 
